...
@Service
/*
 1)помечаем bean-компоненты для указания, что они содержатбизнес-логику("Как продукт будет работать технически?")
      JavaBeans — классы в языке Java, написанные по определённым правилам. Они используются для объединения
      нескольких объектов в один для удобной передачи данных.
 2)объекты многократного использования без сохранения состояния, которые используются для выполнения
      операций бизнес-логики над другими объектами, передаваемыми в качестве аргументов.
 */
@Transactional
/*
...
— это соединение привязывается к “текущему потоку Thread” при помощи ThreadLocal (Класс ThreadLocal предоставляет
 локальные переменные потока. Каждый поток имеет свою собственную инициализированную копию переменной)
 */
