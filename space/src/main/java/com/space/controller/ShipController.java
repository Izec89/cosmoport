...
// наследуется от аннотации @Component (тот же Component, но с рассширенными возможностями) когда "ComponentScan
// видит аннотацию @Controller она создает Bean(объект)  из этого помеченного java - класса
@RequestMapping("/rest")
...
public class ShipController {
    ...
    public List<Ship> getShipsList(@RequestParam(value = "name", required = false) String name, // аннотация @RequestParam предназначена чтобы извлекать только параметры из поступающего GET запроса,
                                   ...
                                   // @RequestParam ждет что мы в URL передадим параметр (/ships?name=Orvill) который она внедрит в переменную, и есди параметров в URL нет то выдаст ошибку,
    ...
    public ResponseEntity<Ship> updateShip(@PathVariable("id") Long id, @RequestBody Ship ship) { //PathVariable связывает
